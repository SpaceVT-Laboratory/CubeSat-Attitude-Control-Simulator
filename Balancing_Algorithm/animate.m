...
%*********************************************************************
%
% NOTE: USER MUST FIRST RUN CSACS_Sim.m TO GENERATE 
%       TRAJECTORY AND MOTOR SPEED DATA FOR animate.m
%
% Generate timeseries for animate.m
euler          = timeseries(y(1:3,:)'*r2d,tt','Name','euler');
rxnwheel_speed = timeseries(y(1:4,:)'*0,tt','Name','rxnwheel_speed');
time           = timeseries(tt',tt','Name','time');
%
%*********************************************************************
...
% UNCOMMENT TO SAVE SIMULATION TO VIDEO FILE:
% video = VideoWriter('CSACSSim_Test4.avi','Motion JPEG AVI');
% open(video)
% writeVideo(video,F);
% close(video)